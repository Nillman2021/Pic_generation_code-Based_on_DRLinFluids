"20250518"
# 导入paraview库,并使用其进行某caseC场的批量生成，这个文件会生成常规的彩色图文件
...
def pic_generation_openfoam(env_name, episode_no, episode_reward):
    ...
    :param fill_factor: 数据区域填充系数(0.9-1.0)，默认0.98表示填充98%的图像区域
    """
    # 创建视图
    ...
    boundaryDisplay.AmbientColor = [1.0, 0.0, 0.0]
    boundaryDisplay.DiffuseColor = [1.0, 0.0, 0.0]

    # 获取所有时间步
    timesteps = openfoam.TimestepValues

    # 创建C场的表面表示
    surface = ExtractSurface(Input=openfoam)
    surfaceDisplay = Show(surface, renderView1)
    surfaceDisplay.Representation = 'Surface'
    surfaceDisplay.ColorArrayName = ['POINTS', 'C']
    surfaceDisplay.LookupTable = GetColorTransferFunction('C')

    # 创建颜色图例
    colorLUT = GetColorTransferFunction('C')
    colorLUT.ApplyPreset('Blue to Red Rainbow', True)
    ...
    # 计算精确缩放比例（优先高度方向填充）
    scale_factor = (image_height/data_height) * 1.1  # 增加10%额外缩放
    ...
    for i, t in enumerate(timesteps):
        ...
        Render()
        SaveScreenshot(os.path.join(output_path, f"Pic.{i:04d}.png"), 
